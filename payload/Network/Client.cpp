...
void NetworkClient::handlePacket(const std::string& packet) {
    ...
    // 1. Ловим команды от сервера: "API: <cmd_id>"
    const std::string apiPrefix = "API: ";
    if (cleanPacket.rfind(apiPrefix, 0) == 0) {
        std::string cmdId = cleanPacket.substr(apiPrefix.length());
        // Добавляем в очередь исполнения
        CommandManager::Instance().AddCommand(cmdId);
        return;
    }

    // 2. Ловим подтверждения (если сервер шлет "ACK: <cmd_id>" или "<cmd_id>_success")
    // Допустим, сервер шлет "ACK: cmd_id"
    const std::string ackPrefix = "ACK: ";
    if (cleanPacket.rfind(ackPrefix, 0) == 0) {
        std::string confirmedId = cleanPacket.substr(ackPrefix.length());
        CommandManager::Instance().ConfirmSuccess(confirmedId);
        return;
    }
    
    // Альтернативный вариант подтверждения (как в твоем черновике)
...
void NetworkClient::run() {
    ...
    while (isRunning) {
        if (sock == -1) {
            ...
            LOGD(">>> Connected <<<");
            CommandManager::Instance().Clear(); // Очищаем очередь при новом подключении
        }
